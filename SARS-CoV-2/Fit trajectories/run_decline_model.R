...
df_merged <- readRDS(file="trajectories.RDS") 
df_merged <- df_merged %>%
  filter(copy_exist)
...
initf <- function(chain_id) {
 list(
   v_s = 3 + (runif(1) - 0.5),
   vgrow = array(c(15, 0.5), dim = c(2, NG)),
   v_sd = c(2, 0.5), fp_ct_sd=1.51+(runif(1)-0.5)
 )
}

fit = sampling(model, chains=8, cores=8, data=data.stan,
           iter=10000, init=initf, init_r=0.1, warmup=3000, seed=497101, thin=2,
