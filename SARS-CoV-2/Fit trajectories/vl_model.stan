functions {
  real logVL(real t, row_vector vp) {
    real vl;
    vl=log(vp[1])+log(1/vp[2]+1/vp[3])-log_sum_exp(-log(vp[3])-1/vp[2]*t,-log(vp[2])+1/vp[3]*t); 
    return vl;
  }
}
...
data {
  ...
  // hyperpriors
  vector<lower=0>[3] hp_v; // hyperprior on kinetic parameter group means
...
generated quantities {
  ...
  vector<lower=0>[62*N] pred_vl; // VL trajectory for each subject
  int i;
  real tpi;
  real vl;
  vector<lower=0>[N] auc;
  real<lower=0> tot_vl;
