data {
  ...
  int<lower=0, upper=1> ind_vacc[N];
  //int con_vacc[N];
...
transformed parameters {
  ...
  real<lower=0, upper=1> pbar_unvacc[N];
  for (n in 1:nvoc){
    zeta[n] = zeta_mu[n] + hp_sd[n]*zeta_sd[n];
  }
  
  for (n in 1:N){
    pbar_vacc[n] = inv_logit(beta*d[n] + gamma_ind[index_age[n]] + zeta[voc[n]] + kappa*antiviral[n]+delta*chronic[n]+nu*ind_vacc[n]);
    pbar_unvacc[n] = inv_logit(alpha + beta*d[n] + gamma_ind[index_age[n]] + zeta[voc[n]] + kappa*antiviral[n]+delta*chronic[n]+nu*ind_vacc[n]);
...
model {
  ...
  for (n in 1:N){
    ...
    target += beta_binomial_lpmf(trans_unvacc[n] |  HH_unvacc[n], theta_unvacc*pbar_unvacc[n]/(1-pbar_unvacc[n]), theta_unvacc);
...
generated quantities {
  ...
  c = 0;
   for (tr in -20:35){
     c = c+1;
     // Vaccinated Delta
     x_gen[c] = inv_logit( alpha + beta*tr/10+gamma_ind[1]+zeta[1]);
   ...
   real log_lik[N];
   for (n in 1:N){
   real ll_unvacc = beta_binomial_lpmf(trans_unvacc[n] |  HH_unvacc[n], theta_unvacc*pbar_unvacc[n]/(1-pbar_unvacc[n]), theta_unvacc);
   real ll_vacc =  beta_binomial_lpmf(trans_vacc[n] |  HH_vacc[n], theta_vacc*pbar_vacc[n]/(1-pbar_vacc[n]), theta_vacc);
   log_lik[n] = ll_unvacc + ll_vacc;
   }
}
